...
```
After reading the article titled “When Databases Get to Define Family” that discusses the role of social expectations in marriage for the Pakistan’s National Database and Registration Authority (NADRA) it has become clear that value systems are embedded into databases and data systems. In this article, the two key social expectations that are embedded into Pakistan’s National Database and Registration Authority is the requirement of kinship to link two households together and gender identities. In order for an individual to receive the Computerized National ID Card (CNIC) it is required to show proof of both the marital status of your parents as well as to fit into one of the defined definition of gender within the database. By having this two key social expectations, it has hindered number of Pakistan individuals from obtaining the Computerized National ID Card (CNIC). Some examples they provided in the article demonstrate how an individual was not able to obtain the CNIC due to their mother not taking the fathers last name leading to no proof of kinship between two households and a single mother did not qualify due to their inability to provide proof of the father. 
After reflecting upon this article, it is clear that social structures and marginalization are important factors that need to be considered while databases are being created. Social structure and marginalization, examples from the article, are being embedded into databases based on assumptions and in some cases intentional exclusion if no plan to change the database is scheduled. For social structure, databases are being built on information that may be out of date and rely on the traditional definitions such as family, citizenship, or employment status, which may not reflect the diversity of real social relationships. In marginalization, individuals or groups who not fit within societal norms can become invisible or misclassified within databases. When designing databases, it is important to consider the diverse populations within society to ensure that individuals are not unnecessarily excluded when inclusion is possible.
```
